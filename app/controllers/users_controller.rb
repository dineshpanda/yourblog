class UsersController < ApplicationController
  def index
    ...
    @users = @q.result(:distinct => true).includes(:articles).page(params[:page]).per(10)
  ...
  def create_row
    ...
    @user.age = params.fetch("age")
    @user.last_name = params.fetch("last_name")
    @user.status = params.fetch("status")
    @user.city = params.fetch("city")
